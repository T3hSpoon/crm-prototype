...
- **Global scope**: `C:/gh-repos/eld/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/gh-repos/eld/.claude/notes/`)
...
2. Glob `C:/gh-repos/eld/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/gh-repos/eld/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/gh-repos/eld/.claude/notes/` — works in any directory
